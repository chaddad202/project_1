<!-- resources/views/category_edit.blade.php -->
...
<div class="edit-category-container">
    ...
    <form method="POST" action=" /category_update/{{$category->id}}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" id="title_ar" name="title_ar" value="{{ $category->title_ar }}">
        </div>
        <div class="form-group">
            <label for="title_en">Title in English</label>
            <input type="text" id="title_en" name="title_en" value="{{ $category->title_en }}">
        </div>

        <!-- إدخال الوصف -->
        <div class="form-group">
            <label for="description_ar">Description in Arabic</label>
            <textarea id="description_ar" name="description_ar" rows="4">{{ $category->description_ar }}</textarea>
        </div>
        <div class="form-group">
            <label for="description_en">Description in English</label>
            <textarea id="description_en" name="description_en" rows="4">{{ $category->description_en }}</textarea>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="image-preview-container">
                @if($category->photo)
                    <img id="current-photo" src="{{url('storage/' . str_replace('public/', '', $category->photo))}}" alt="Category Photo">
                @endif
        ...
        <div class="form-buttons">
            ...
            <a href="{{ route('category.show') }}" class="btn-cancel">Cancel</a>
        </div>
    </form>
</div>
